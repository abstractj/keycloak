...
function run-server-tests() {
    ...
    mvn -s $HOME/maven-settings.xml install -B -nsu -Pauth-server-wildfly -DskipTests
...
COMPILING_PID=$!
TMPFILE=`mktemp`
if ! mvn -s $HOME/maven-settings.xml install -B -nsu -Pdistribution -DskipTests -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn &> "$TMPFILE"; then
    cat "$TMPFILE"
    exit 1
fi
kill $COMPILING_PID
...
if [ $1 == "crossdc-server" ]; then
    cd testsuite/integration-arquillian
    mvn -s $HOME/maven-settings.xml install -B -nsu -Pauth-servers-crossdc-jboss,auth-server-wildfly,cache-server-infinispan -DskipTests
...
if [ $1 == "crossdc-adapter" ]; then
    cd testsuite/integration-arquillian
    mvn -s $HOME/maven-settings.xml install -B -nsu -Pauth-servers-crossdc-jboss,auth-server-wildfly,cache-server-infinispan,app-server-wildfly -DskipTests
